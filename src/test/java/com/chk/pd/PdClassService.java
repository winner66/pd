...
//            }
        ...
        //将军级扩展为GJB,QJ G+,G,J 4个
        for (CasRsp value : lev2Map.values()) {
            List<CasRsp> lev2_2 = value.getChildren();
            for (int i = 0; i < lev2_2.size(); i++){
                CasRsp cas = lev2_2.get(i);
                if (cas.getLabel().equals("军级")){
                    String qa1 = "GJB";
                    CasRsp c1 =  new CasRsp("国军标(GJB)", cas.getValue() + "__" + qa1,true);
                    c1.getChildren().addAll(copyCasRsp(cas.getChildren(), qa1));

                    String qa2 = "QJ,G+";
                    CasRsp c2 =  new CasRsp("企军标(QJ,G+)", cas.getValue() + "__" + qa2,true);
                    c2.getChildren().addAll(copyCasRsp(cas.getChildren(), qa2));

                    String qa3 = "G";
                    CasRsp c3 =  new CasRsp("七专(G)", cas.getValue() + "__" + qa3,true);
                    c3.getChildren().addAll(copyCasRsp(cas.getChildren(), qa3));

                    String qa4 = "J";
                    CasRsp c4 =  new CasRsp("普军(J)", cas.getValue() + "__" + qa4,true);
                    c4.getChildren().addAll(copyCasRsp(cas.getChildren(), qa4));
        ...
        for (CasRsp value : lev2Map.values()) {
            List<CasRsp> lev2_2 = value.getChildren();
            for (Iterator<CasRsp> i = lev2_2.iterator();i.hasNext();){
                CasRsp cas = i.next();
                if (cas.getLabel().equals("军级")){
                    i.remove();
                }
    ...
    @SneakyThrows
    List<CasRsp> copyCasRsp(List<CasRsp> rsps, String qa){
        List<CasRsp> list = new ArrayList<>();
        for (CasRsp rsp : rsps) {
            CasRsp c = new CasRsp(rsp.getLabel(), rsp.getValue()+ "__" + qa, false);
            list.add(c);
        }
        return list;
    }
